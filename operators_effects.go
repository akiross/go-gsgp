...
func (p ByFitness) Len() int { return int(p.num_ind) }

func (p ByFitness) Swap(i, j int) {
	p.individuals[i], p.individuals[j] = p.individuals[j], p.individuals[i]
	sem_train_cases[i], sem_train_cases[j] = sem_train_cases[j], sem_train_cases[i]
	sem_test_cases[i], sem_test_cases[j] = sem_test_cases[j], sem_test_cases[i]
}
...
var (
	...
	config = Config{
		...
		of_train:             flag.String("out_file_train_fitness", "op_eff_fit_train.txt", "Path for the output file with train fitness data"),
		of_test:              flag.String("out_file_test_fitness", "op_eff_fit_test.txt", "Path for the output file with test fitness data"),
	...
	ind_type []byte // Type of indiviudal (seeded, good, bad)
...
func main() {
	...
	for k := 0; k < *config.population_size; k++ {
		fmt.Fprintln(fitness_train, fit[k], fit_new[k])
		fmt.Fprintln(fitness_test, fit_test[k], fit_test_new[k])
	}
}
